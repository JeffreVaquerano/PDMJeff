...
class MainActivity : AppCompatActivity() {
    ...
    private fun displayResult(bmi: Float){
        ...
        infoTextView.text = "(El rango normal es entre: 18.5 - 24.9)"
        var informationResult = ""
        var color = 0

        when{
            bmi < 18.50 -> {
                informationResult = "Bajo de Peso"
                color = R.color.under_weight
            }
            bmi in 18.50..24.99 -> {
                informationResult = "Saludable"
                color = R.color.normal_weight
            }
            bmi in 25.00..29.99 -> {
                informationResult = "Sobre Peso"
                color = R.color.over_weight
            }
            bmi > 18.50 -> {
                informationResult = "Obesidad"
                color = R.color.obese
            }
        }
